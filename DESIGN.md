...
# UNIVERSAL VOXEL REVERSE-ENGINEERING SPECIFICATION

## Objective

Convert any real-world object into a voxel construction plan with zero ambiguity.

The output must be deterministic, reproducible, measurable, and buildable.

No artistic interpretation.

No assumptions.

No skipped dimensions.

Every voxel must be derivable from the specification.

---

# Phase 1 — Analyze Reference Object
...
## Identify Major Assemblies

Break object into major components.
...
E = Decorative Components
F = Lighting
```

Never voxelize the entire object at once.

Always voxelize assemblies separately.

---

# Phase 2 — Define Bounding Box
...
| Height          | \_\_\_    |
| Voxel Size      | \_\_\_ mm |
| Physical Width  | \_\_\_ mm |
| Physical Depth  | \_\_\_ mm |
| Physical Height | \_\_\_ mm |

---

# Phase 3 — Primitive Shape Decomposition

Reduce object into primitives.
...
For every assembly identify:

```text
Assembly A = Cylinder
Assembly B = Cube
Assembly C = Cone
Assembly D = Extrusion
```

---

# Phase 4 — Orthographic Construction
...
These become the master reference.

No building begins before orthographic approval.

---

# Phase 5 — Layer Slicing

Treat model like a CT scan.

Slice model horizontally.
...
# Phase 6 — Coordinate System

All voxels use coordinates.
...
# Phase 7 — Color Assignment

Assign colors before building.
...
3 Build Support Structure

4 Build Functional Components
...
No parallel construction unless specified.
...
Check every component against color schedule.

---

# Phase 10 — Manufacturing Package
...
If a builder must guess, the specification is incomplete.

A complete voxel specification allows two independent engineers to build identical models with no communication beyond the document.
    ...
    <style>
      * {
      ...
      .variation-btn.active {
        ...
        border-color: rgba(210, 105, 30, 0.9);
        color: #fff;
      }
      .variation-btn[data-variation="1"].active {
        background: rgba(192, 192, 192, 0.15);
        border-color: rgba(192, 192, 192, 0.5);
        color: #c0c0c0;
      }
      .variation-btn[data-variation="2"].active {
...
const categoryMatPresets = {
  ...
  neck: { rough: 0.8, metal: 0.05, clearcoat: 0, physical: false },
  frets: { rough: 0.2, metal: 0.9, clearcoat: 0.5, physical: true },
  strings: { rough: 0.2, metal: 1.0, clearcoat: 0.8, physical: true },
  pickups: { rough: 0.6, metal: 0.2, clearcoat: 0, physical: false },
};
...
function updateMorph() {
  ...
  voxels.forEach((im) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const bz = baseSnapData
        ? baseSnapData.positions[i3 + 2]
        : basePos[i3 + 2];
      ...
      const ny =
        basePos[i3 + 1] + (toData.positions[i3 + 1] - basePos[i3 + 1]) * localT;
      const nz =
        basePos[i3 + 2] + (toData.positions[i3 + 2] - basePos[i3 + 2]) * localT;
...
function updateRepulsion(dt, camera) {
  const mouseIdle =
    (performance.now() - lastMouseMoveTime) / 1000 > mouseIdleTimeout;
  ...
  const validHit =
    rawHit && _planeIntersect.distanceTo(_bboxCenter) < maxProxyDist;

  if (validHit) {
    if (!_hasSmoothedHit) {
      _smoothHitPoint.copy(_planeIntersect);
...
for (let i = 0; i < dustCount; i++) {
  ...
  dustPos[i * 3 + 1] = Math.random() * 35 - 5;
...
try {
  ...
  aoNode.thickness.value = 1.6;
  currentOutput = vec3(currentOutput.rgb.mul(aoNode.getTextureNode().r)).toVec4(
    currentOutput.a,
  );
} catch (e) {
  console.warn("GTAO:", e.message);
}
...
try {
  ...
  ssrNode.resolutionScale = 0.25;
  ssrNode.thickness.value = 0.2;
  ssrNode.maxDistance.value = 4.0;
  const s = uniform(0.25);
  currentOutput = vec3(
    currentOutput.rgb.add(ssrNode.getTextureNode().rgb.mul(s)),
  ).toVec4(currentOutput.a);
} catch (e) {
  console.warn("SSR:", e.message);
}
