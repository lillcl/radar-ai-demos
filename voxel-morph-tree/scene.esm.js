...
import {
  ...
  metalness,
  roughness
} from "https://cdn.jsdelivr.net/npm/three@0.183.2/build/three.tsl.js";
import { OrbitControls } from "https://cdn.jsdelivr.net/npm/three@0.183.0/examples/jsm/controls/OrbitControls.js";
import { RGBELoader } from "https://cdn.jsdelivr.net/npm/three@0.183.0/examples/jsm/loaders/RGBELoader.js";
var ao;
var denoise;
var ssrModule;
var bloomModule;
try {
  const gtaoMod = await import("https://cdn.jsdelivr.net/npm/three@0.183.0/examples/jsm/tsl/display/GTAONode.js");
  ao = gtaoMod.ao;
...
try {
  const denoiseMod = await import("https://cdn.jsdelivr.net/npm/three@0.183.0/examples/jsm/tsl/display/DenoiseNode.js");
...
try {
  ssrModule = await import("https://cdn.jsdelivr.net/npm/three@0.183.0/examples/jsm/tsl/display/SSRNode.js");
  console.log("SSR module loaded successfully");
} catch (e) {
  console.warn("SSR addon module not available:", e.message);
}
try {
  bloomModule = await import("https://cdn.jsdelivr.net/npm/three@0.183.0/examples/jsm/tsl/display/BloomNode.js");
  console.log("Bloom module loaded successfully");
} catch (e) {
  console.warn("Bloom addon module not available:", e.message);
}
var scene = new THREE.Scene();
var sceneContainer = document.querySelector(".hero-scene") || document.getElementById("root")?.parentElement || document.body;
var containerW = () => sceneContainer.clientWidth || window.innerWidth;
var containerH = () => sceneContainer.clientHeight || window.innerHeight;
...
function createSkyEnvironment() {
  ...
  sunMat.colorNode = color(16777198).mul(float(2));
...
var voxelSize = 1;
var gap = 0;
var step = voxelSize + gap;
var voxelGeo = new THREE.BoxGeometry(
  voxelSize,
  voxelSize,
  voxelSize,
  1,
...
var trunkColors = ["#4a3728", "#3d2e20", "#5c4535", "#2e2218", "#6b5444"];
var leafColors = [
  "#e63c2e",
  "#d4452f",
  "#f05a3a",
...
var voxels = [];
var categoryBatches = {};
var occupiedPositions = /* @__PURE__ */ new Set();
...
var VARIATION_BLOOM = [0.5, 0.15, 0.25];
var isMorphing = false;
var morphDuration = 1.8;
var hillData = [];
var undersideData = [];
...
var topSurfaceSet = /* @__PURE__ */ new Set();
...
for (let y = -2; y >= -14; y--) {
  ...
  for (let x = -10; x <= 10; x++) {
    for (let z = -8; z <= 8; z++) {
      ...
      const noise = pseudoNoise(x * 0.8 + depth * 0.3, z * 0.8 - depth * 0.2) * (1 + depth * 0.08);
...
var canopyCenterX = 0;
var canopyCenterY = 20;
var canopyCenterZ = 0;
var canopyRadiusH = 6.5;
var canopyRadiusV = 4.5;
for (let x = -8; x <= 8; x++) {
  for (let y = 15; y <= 26; y++) {
    for (let z = -7; z <= 7; z++) {
...
var grassTuftColors = ["#3a8530", "#4a9540", "#2d7020", "#5aad50", "#3d8a35"];
var rockSet = new Set(rockPositions.map((r) => `${r.x},${r.z}`));
...
var mushroomColors = ["#f5e6c8", "#e8d5b0", "#d4c49a", "#c9b88e"];
Object.entries(grassTopMap).forEach(([key, topY]) => {
  const [gx, gz] = key.split(",").map(Number);
...
var instanceData = /* @__PURE__ */ new Map();
var _islandBBox = new THREE.Box3();
{
  const dummy = new THREE.Object3D();
...
function generateSnowyPineOffsets() {
  ...
  const pineOccupied = /* @__PURE__ */ new Set();
  voxels.forEach((im) => {
...
function generateCherryBlossomOffsets() {
  ...
  const cherryOccupied = /* @__PURE__ */ new Set();
  voxels.forEach((im) => {
    const data = instanceData.get(im);
    if (!data) return;
    const count = im.count;
...
var morphFrom = 0;
var morphTo = 0;
var morphBasePositions = null;
var morphBaseColors = null;
...
function updateMorph() {
  ...
  const elapsed = (performance.now() - morphStartTime) / 1e3;
...
var dustVelocities = new Float32Array(dustCount * 3);
var dustSizes = new Float32Array(dustCount);
var dustOpacities = new Float32Array(dustCount);
var dustLifetimes = new Float32Array(dustCount);
...
var dustMat = new THREE.PointsNodeMaterial({
  transparent: true,
  depthWrite: false,
  blending: THREE.AdditiveBlending,
  sizeAttenuation: true
});
dustMat.colorNode = color(16777164);
...
var fallingLeafCount = 40;
var leafQuadGeo = new THREE.PlaneGeometry(0.5, 0.5);
var leafQuadMat = new THREE.MeshBasicNodeMaterial({
  transparent: true,
  depthWrite: false,
  side: THREE.DoubleSide
});
leafQuadMat.colorNode = color(15224874);
leafQuadMat.opacityNode = float(0.8);
var fallingLeaves = new THREE.InstancedMesh(
  leafQuadGeo,
  leafQuadMat,
  fallingLeafCount
);
...
var leafState = [];
var leafDummy = new THREE.Object3D();
var leafParticleColors = [
...
function updateParticles(dt) {
  ...
  leafQuadMat.colorNode = color(16777215);
  ...
  for (let i = 0; i < fallingLeafCount; i++) {
    ...
    const alpha = lifeFrac < 0.1 ? lifeFrac / 0.1 : lifeFrac > 0.85 ? (1 - lifeFrac) / 0.15 : 1;
...
var repulsionStrength = 18;
var returnSpeed = 2.5;
var lastMouseMoveTime = 0;
var mouseIdleTimeout = 0.08;
var mouseActive = false;
var _smoothHitPoint = new THREE.Vector3(9999, 9999, 9999);
var _hasSmoothedHit = false;
var hitSmoothSpeed = 12;
var _bboxCenter = new THREE.Vector3();
_islandBBox.getCenter(_bboxCenter);
var _rayPlane = new THREE.Plane();
var _planeIntersect = new THREE.Vector3();
...
function updateRepulsion(dt) {
  ...
  const mouseIdle = (now - lastMouseMoveTime) / 1e3 > mouseIdleTimeout;
  ...
  if (validHit) {
    ...
    if (!_hasSmoothedHit) {
      ...
      const smoothFactor = 1 - Math.exp(-hitSmoothSpeed * Math.min(dt, 0.05));
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (hasHit) {
        ...
        if (dist < repulsionRadius && dist > 0.01) {
          ...
          const pulseAmount = Math.sin(pulseTime * 3e-3 + pulsePhase) * 0.15 + Math.sin(pulseTime * 17e-4 + pulsePhase * 0.6) * 0.1;
          const breathScale = 1 + pulseAmount * falloff;
          const rx = randDirs[i * 3];
          const ry = randDirs[i * 3 + 1];
          const rz = randDirs[i * 3 + 2];
          const radialMix = 0.6;
          const mx = _dir.x * radialMix + rx * (1 - radialMix);
          const my = _dir.y * radialMix + ry * (1 - radialMix);
          const mz = _dir.z * radialMix + rz * (1 - radialMix);
      ...
      const activeSpeed = hasHit ? 8 : returnSpeed;
      const lerpFactor = 1 - Math.exp(-activeSpeed * dtClamped);
      ...
      if (Math.abs(newX - curX) > 1e-4 || Math.abs(newY - curY) > 1e-4 || Math.abs(newZ - curZ) > 1e-4) {
        offsets[i * 3] = newX;
        offsets[i * 3 + 1] = newY;
        offsets[i * 3 + 2] = newZ;
        ...
        needsUpdate = true;
      } else if (Math.abs(curX) > 1e-4 || Math.abs(curY) > 1e-4 || Math.abs(curZ) > 1e-4) {
        offsets[i * 3] = newX;
        offsets[i * 3 + 1] = newY;
        offsets[i * 3 + 2] = newZ;
        im.getMatrixAt(i, _mat4);
...
var voxelMat = voxelMats[0];
var ambientLight = new THREE.AmbientLight(16772829, 0.5);
ambientLight.name = "ambientLight";
scene.add(ambientLight);
var mainLight = new THREE.DirectionalLight(16774624, 2.5);
...
scene.add(mainLight);
var softShadowLight = new THREE.DirectionalLight(16772829, 0.6);
...
softShadowLight.shadow.bias = 1e-4;
...
var fillLight = new THREE.DirectionalLight(8961023, 1);
fillLight.name = "fillLight";
fillLight.position.set(-5, 8, -3);
scene.add(fillLight);
var rimLight = new THREE.PointLight(16755302, 1.5, 30);
rimLight.name = "rimLight";
rimLight.position.set(-4, 12, -5);
scene.add(rimLight);
var accentLight = new THREE.PointLight(16746564, 1.2, 25);
    ...
    try {
      ...
      if (denoise) {
        try {
          ...
          denoisePass.sigma.value = 30;
          denoisePass.kSigma.value = 5;
...
var aoBaseSettings = { samples: 16, radius: 0.6 };
var AO_NEAR_DIST = 3;
var AO_FAR_DIST = 8;
var lastAdaptiveDist = -1;
...
function animate() {
  ...
  const dt = (now2 - lastTime) / 1e3;
  lastTime = now2;
  frameCount++;
  const now = performance.now();
  if (now - lastFpsTime >= 500) {
    const fps = Math.round(frameCount / ((now - lastFpsTime) / 1e3));
...
var defaultSwatch = document.querySelector(
...
if (shadowToggle) {
  shadowToggle.addEventListener("change", () => {
    ...
    if (track)
      track.style.background = shadowsEnabled ? "#3cb371" : "rgba(255,255,255,0.15)";
...
var shadowResSelect = document.getElementById("shadow-res");
if (shadowResSelect) {
...
var aoToggle = document.getElementById("ao-toggle");
var aoEnabled = true;
var aoSavedThickness = 1.6;
if (aoToggle) {
...
var ssrToggle = document.getElementById("ssr-toggle");
var ssrEnabled = true;
var ssrSavedStrength = 0.25;
if (ssrToggle) {
  ssrToggle.addEventListener("change", () => {
    ...
    if (track)
      track.style.background = ssrEnabled ? "#3cb371" : "rgba(255,255,255,0.15)";
...
var bloomToggle = document.getElementById("bloom-toggle");
var bloomEnabled = true;
var bloomSavedStrength = 0.5;
if (bloomToggle) {
  bloomToggle.addEventListener("change", () => {
    ...
    if (track)
      track.style.background = bloomEnabled ? "#3cb371" : "rgba(255,255,255,0.15)";
...
var randomizeBtn = document.getElementById("btn-randomize");
if (randomizeBtn) {
